...
Las restricciones usualmente son nuestra(s) hipótesis nula(s) y en ese caso las usamos para eliminar algunos parámetros del conjunto usado en el modelo completo. Naturalmente llamamos al modelo resultante simplemente el modelo restringido.[¿piensas que sería válido restringir un modelo con valores de algún parámetro con un valor que no sea cero?]{style="color:GoldenRod"}
...
[¿No es este un modelo de regresión simple? ¿explicalo?]{style="color:GoldenRod"}

![image.png](../../../Figuras/ANOVA_modelo_hojas.png)
...
Tenemos las dos situaciones experimentales que se ilustran en la figura. El análisis de estas situaciones experimentales sugiere dos aproximaciones distintas.

![image.png](../../../Figuras/retricciones_a_la_aleatorizaci%C3%B3n_1.png)

![](images/retricciones_a_la_aleatorizaci%C3%B3n_1.png)

Veamos, [¿qué modelos propondrías para analizarlos?]{style="color:GoldenRod"}
...
[¿cuál piensas es el efecto de restringir el proceso de aleatorización en la asignación de los tratamientos?]{style="color:GoldenRod"}.

Anderson propuso incorporar en el modelo términos que dieran cuenta de restricciones en la aleatorización de las unidades experimentales. Llamó a estos términos "errores de restricción", pues también sugirió que deberían ser términos de *efectos aleatorios*. Así, el modelo de bloques se

$$
y_{ijkm} = \mu + Z_{i} + B_{j} + \delta_{k(j)} + \varepsilon_{m(ijk)}
$$

En donde los términos tienen la misma interpretación de arriba, y los cambios son: \* $\delta_{k(j)}$ es el componente de variación aleatoria introducido por la restricción a la aleatorización (no estimable). \* $\varepsilon_{m(ijk)}$ es el componente aleatorio derivado de la presición de las mediciones.
